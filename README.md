...
### Lexer
---
`Lexer` reads character sequences in the provided file, trying to find input patterns called `lexemes`, that are represented internally by a corresponding `token`. This `token` is later provided to the parser. In case of an invalid token, it is reported and compilation stops. For this project `ocamllex lexer` is used. `Lexer` file can be found at [**`src/parsing/Lexer.mll`**](./blob/master/src/parsing/Lexer.ml).
...
For this project `menhir parser` is used. `Parser` and `AST` files can be found at [**`src/parsing/Parser`**](./blob/master/src/parsing/Parser.ml) and [**`src/utils/Ast.ml`**](./blob/master/src/utils/Ast.ml), respectively.
...
> Examples of programs and generated files can be found at [**`examples/programs`**](./blob/master/examples/programs) directory
