...
let collect (venv: venv) (tenv: tenv) (tast: tast): CT.contree =

  (** [collect_defs contree ds] returns the ConstraintTree.def list [contree] from TypedAst.def list [ds] *)
  let rec collect_defs contree = function
    | [] -> rev contree
    | d :: ds -> match d with
      | TA.LetDefNonRec _ | TA.LetDefRec _ ->
        collect_defs ((collect_let_def d) :: contree) ds
      | TA.TypeDef _ ->
        collect_defs contree ds

  (** [collect_let_def d] returns the ConstraintTree.def from TypedAst.def [d] *)
  and collect_let_def = function
    | TA.LetDefNonRec { decs } ->
      CT.LetDefNonRec { decs = collect_decs venv decs }
    | TA.LetDefRec { decs } ->
      CT.LetDefRec { decs = collect_decs venv decs }
    | TA.TypeDef { loc } ->
      internal_error loc "collect_let_def applied to TypedAst.TypeDef"

  (** [collect_let_def_local_venv venv d] returns the augmented venv from [venv]
      and the ConstraintTree.def from TypedAst.def [d] -- used from let...in expressions *)
  and collect_let_def_local_venv venv = function
    | TA.LetDefNonRec { decs } ->
      let local_venv = List.fold_left augment_venv_dec venv decs in
      let condecs = collect_decs local_venv decs in
      local_venv, CT.LetDefNonRec { decs = condecs }
    | TA.LetDefRec { decs } ->
      let local_venv = List.fold_left augment_venv_dec venv decs in
      let condecs = collect_decs local_venv decs in
      local_venv, CT.LetDefRec { decs = condecs }
    | TA.TypeDef { loc } ->
      internal_error loc "collect_let_def_local_venv applied to TypedAst.TypeDef"

  (** [collect_decs venv condecs] returns the ConstraintTree.dec list from
      venv [venv] and TypedAst.dec list [condecs] *)
  and collect_decs venv condecs =
    let rec aux acc = function
      | [] -> rev acc
      | d :: ds -> match d with
        | TA.ConstVarDec _ | TA.FunctionDec _ | TA.ArrayDec _ ->
          aux ((collect_dec venv d) :: acc) ds
        | TA.MutVarDec _ ->
          aux acc ds
    in
    aux [] condecs
  ...
  and collect_dec venv = function
    | TA.ConstVarDec { ty; name_sym; value } ->
      let convalue = collect_expr venv value in
    ...
    | TA.FunctionDec { ty; name_sym; params; body; loc } ->
      ...
      let conbody = collect_expr local_venv body in
    ...
    | TA.ArrayDec { dims_len_exprs } ->
      let conexprs = List.map (collect_expr venv) dims_len_exprs in
  ...
  and collect_expr venv e =
    ...
    and collect_clause = function
      | TA.BasePattClause { base_pattern; expr } ->
        ...
        let conexpr = collect_expr local_venv expr in
