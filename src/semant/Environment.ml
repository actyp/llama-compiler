...
let io_functions =
  [
    ...
    ("read_int"    , fun_ty [T.INT] T.UNIT);
    ("read_bool"   , fun_ty [T.BOOL] T.UNIT);
    ("read_char"   , fun_ty [T.CHAR] T.UNIT);
    ("read_float"  , fun_ty [T.FLOAT] T.UNIT);
    ("read_string" , fun_ty [string_ty ()] T.UNIT);
  ]
